...
#include <cstdlib> //使用malloc和free必須加 
...
//3.用malloc動態分配二維陣列
    char**matrix_malloc=(char**)malloc(x*sizeof(char*)); //分配x行
    for (int i=0;i<x;i++){
        matrix_malloc[i]=(char*)malloc((y+1)*sizeof(char)); //字串分配長度y+1含'/0'，'/0'是作為結束符
    }
    
//4.要求使用者輸入的字串放入matrix
    for (int i=0;i<x;i++){
    	cout<<"請輸入第"<<i+1<<"個字串(最多"<<y<<"個字元):";
		cin.getline(matrix_malloc[i],y+1);
    }

//5.輸出字串逐一放入動態二維陣列中
     cout <<"Part1_逐一放入原字串:"<<endl;
	for(int i=0;i<x;i++) {
	   cout <<matrix_malloc[i] <<endl;
    } 
    
//7.輸出排序的結果
    bubbleSort(matrix_malloc, x); //使用bubble sort完成條件:字串第一個字元依大小排序 
    std::cout <<"Part2_經排序後的字串為:"<<endl;
	for(int i=0;i<x;i++) {
	   cout <<matrix_malloc[i] <<endl;
    }
...
//8.手動釋放動態陣列記憶體，二維陣列需逐行釋放，再釋放行指標
    for(int i=0;i<x;i++) {
	   free (matrix_malloc[i]); //釋放每一行 
    }
    free (matrix_malloc); //釋放行指標 
